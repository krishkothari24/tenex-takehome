/**
 * The suggested starter bucket taxonomy — shown to a new user as opt-in picker chips (not
 * silently auto-created; see `apps/web/src/components/BucketPicker.tsx`), and consumed
 * server-side by the classifier/eval/dev-script code as the canonical default definitions.
 * Lives in `shared` so both the frontend picker UI and the backend can import it directly with
 * no network round trip and no duplicated copy. `description` is sent to the model verbatim if
 * the bucket is created — it's the primary grounding for each label, so edit it as a prompt, not
 * just a UI hint.
 */
