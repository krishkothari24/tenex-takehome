...
test('accepts a well-formed batch and defaults a missing secondaryBucket to null', () => {
  ...
  const parsed = schema.parse({
    classifications: [
      { index: 1, bucket: 'Important', confidence: 0.9, justification: 'Mentions a Friday deadline.', secondaryBucket: null },
      { index: 2, bucket: 'Newsletter', confidence: 0.8, justification: 'Weekly digest from a mailing list.' },
    ],
  });
...
test('rejects a bucket outside the allowed set', () => {
  ...
  assert.throws(() =>
    schema.parse({
      classifications: [{ index: 1, bucket: 'Spam', confidence: 0.9, justification: 'x' }],
    }),
  );
});
...
test('rejects a confidence outside [0,1]', () => {
  ...
  assert.throws(() =>
    schema.parse({
      classifications: [{ index: 1, bucket: 'Important', confidence: 1.4, justification: 'x' }],
    }),
  );
});
...
test('rejects an empty justification', () => {
  ...
  assert.throws(() =>
    schema.parse({
      classifications: [{ index: 1, bucket: 'Important', confidence: 0.9, justification: '   ' }],
    }),
  );
});
